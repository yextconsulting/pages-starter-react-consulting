...
export function isDuplicateFieldValueFilter(
  ...
  if (JSON.stringify(thisFilter.value) !== JSON.stringify(otherFilter.value)) {
    return false;
  }
  return true;
}
 ...
 */
export function isDuplicateStaticFilter(
  thisFilter: StaticFilter,
  otherFilter: StaticFilter
): boolean {
  ...
  return (
    thisFilter.combinator === otherFilter.combinator &&
    thisFilter.filters.length === otherFilter.filters.length &&
    thisFilter.filters.every((t) =>
      otherFilter.filters.some((o) => isDuplicateStaticFilter(t, o))
    ) &&
    otherFilter.filters.every((o) =>
      thisFilter.filters.some((t) => isDuplicateStaticFilter(o, t))
    )
  );
}
 ...
 */
export function parseNumberRangeInput(
 ...
 */
export function clearStaticRangeFilters(
  searchActions: SearchActions,
  fieldIds?: Set<string>
) {
  const selectedStaticRangeFilters =
 ...
 */
export function getSelectedNumericalFacetFields(
  searchActions: SearchActions
): Set<string> {
  const selectedNumericalFacets =
    searchActions.state.filters.facets?.filter(

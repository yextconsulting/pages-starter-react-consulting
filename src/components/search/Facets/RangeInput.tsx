...
import { executeSearch } from "./search-operations";
...
const builtInCssClasses: Readonly<RangeInputCssClasses> = {
  ...
  input:
    "w-full h-9 form-input cursor-pointer border rounded-md focus:ring-0 text-neutral-dark text-sm text-right appearance-none leading-9",
  input___withPrefix: "pl-[2.5rem]",
  input___withoutPrefix: "px-2",
  input___disabled:
    "bg-gray-50 placeholder:text-neutral-light cursor-not-allowed",
  ...
  tooltip:
    "absolute z-10 left-0 whitespace-nowrap rounded shadow-lg p-3 text-sm bg-neutral-dark text-white",
...
export function RangeInput(props: RangeInputProps): JSX.Element | null {
  ...
  const isDisabled = filters.some(
    (filter) => filter.selected && filter.fieldId === fieldId
  );
  ...
  const matchingFilter = findSelectableFieldValueFilter(
    rangeFilter,
    fieldValueFilters
  );
  ...
  function renderInput(
    ...
    return (
      <div className={cssClasses.inputContainer}>
        {inputPrefix && (
          <span className={inputPrefixClasses} aria-hidden="true">
            {inputPrefix}
          </span>
        )}
        <input
    ...
    <div className={cssClasses.rangeInputContainer}>
      ...
      {hasUserInput && (
        <div className={cssClasses.buttonsContainer}>
          <button className={cssClasses.clearButton} onClick={handleClickClear}>
            Clear min and max
          </button>

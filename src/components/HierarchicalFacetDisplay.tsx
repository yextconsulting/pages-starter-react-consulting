...
import {
  ...
  useHierarchicalFacetTree,
} from "../utils/useHierarchicalFacetTree";
...
export default function HierarchicalFacetDisplay({
  ...
  function renderTree(): ReactNode[] {
    ...
    const renderedNodesAndShowMoreButton: ReactNode[] = [
    ...
    while (treePointer) {
      ...
      const selectedHasNoChildren =
        selectedChildNode &&
        Object.values(selectedChildNode.childTree).length === 0;
      ...
      if ((!selectedChildNode && !activeParentNode) || selectedHasNoChildren) {
        renderedNodesAndShowMoreButton.push(
          ...renderAvailableOptions(currentNodes)
        );
...
function AllCategories({
  ...
  const handleClickAllCategories = useCallback(() => {
    facet.options
      .filter((o) => o.selected)
      .forEach((o) =>
        selectFilter({ ...o, fieldId: facet.fieldId, selected: false })
      );
    applyFilters();
    resetShowMore();
  ...
  if (facet.options.find((o) => o.selected)) {
    return (
      <button className={inactiveClassName} onClick={handleClickAllCategories}>
        All Categories /
      </button>
    );
  }

  return <div className={activeClassName}>All Categories</div>;
}
...
function ParentCategory({
  ...
  const deselectChildOptions = useCallback(
...
function CurrentCategory({
  ...
  return (
    <button className={className} onClick={handleClickCurrentCategory}>
      {selectedNode.lastDisplayNameToken}
    </button>
  );
}

...
export function parseHierarchicalFacetTree(
  ...
  const optionsInAscendingLength =
    hierarchicalFacet?.options
      .map((o) => {
        const displayNameTokens = o.displayName
          .split(delimiter)
          .map((s) => s.trim());
        return {
          ...o,
      ...
      .sort(
        (a, b) => a.displayNameTokens.length - b.displayNameTokens.length
      ) || [];
  ...
  optionsInAscendingLength.forEach((o) => {
    ...
    const lastDisplayNameToken =
      displayNameTokens[displayNameTokens.length - 1];
